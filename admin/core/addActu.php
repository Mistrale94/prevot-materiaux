...
if(empty($_POST['title'])){
    ...
    header('Location: ../addActuBack.php');
    exit();
}
if(empty($_POST['date'])){
...
if($error){
    header('Location: ../addActuBack.php?error');
    exit();
}else{
    $newName = 'pic-'.time().'.'.$extFile;
    move_uploaded_file($_FILES['fichier']['tmp_name'],'../data/'.$newName);

    $add = $db->prepare('INSERT INTO actuality (file, title, date, description) VALUES (:file, :title, :date, :description)');
    $add->execute([
        ':file' => $newName,
        ':title' => $_POST['title'],
        ':date' => $_POST['date'],
