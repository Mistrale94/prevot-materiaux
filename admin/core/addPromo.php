...
if(empty($_POST['title'])){
    ...
    header('Location: ../addPromoBack.php');
    exit();
}
if(empty($_POST['date'])){
...
if($error){
    header('Location: ../addPromoBack.php?error');
    exit();
}else{
    $newName = 'pic-'.time().'.'.$extFile;
    move_uploaded_file($_FILES['fichier']['tmp_name'],'../data/'.$newName);

    $add = $db->prepare('INSERT INTO promotion (file, title, date, description) VALUES (:file, :title, :date, :description)');
    $add->execute([
        ':file' => $newName,
        ':title' => $_POST['title'],
        ':date' => $_POST['date'],
