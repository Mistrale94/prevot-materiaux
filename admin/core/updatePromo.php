...
if(isset($_FILES['fichier']) AND !empty($_FILES['fichier']['name'])){
    ...
    $read = $db->prepare('SELECT file FROM promotion WHERE id = :id');
    $read->execute([
        ':id' => $_POST['id']
    ]);
    $modif = $read->fetch();
    ...
    if(in_array($extFile, $tExtOk)){
        ...
        if($move){
            $modFile = $db->prepare('UPDATE promotion SET file = :file WHERE id = :id');
            $modFile->execute([
                ':file' => $newName,
                ':id' => $_POST['id']
            ]);
...
if($error){
    ...
    $mod = $db->prepare('UPDATE promotion SET title = :title, date = :date, description = :description WHERE id = :id');
    $mod->execute([
        ':id' => $_POST['id'],
        ':title' => $_POST['title'],
        ':date' => $_POST['date'],
